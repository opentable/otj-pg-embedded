...
public class TestPostgresSchemaController
{
    ...
    {
        final PostgresController manager = new PostgresController(rootConfig, userConfig, false);

        Assert.assertFalse(checkExists(dbName, userName));
        Assert.assertFalse(manager.exists());

        try {
            manager.create();
            Assert.assertTrue(manager.exists());
            Assert.assertTrue(checkExists(dbName, userName));
        }
        finally {
            manager.drop();
            Assert.assertFalse(checkExists(dbName, userName));
    ...
    private boolean checkExists(final String dbName, final String schemaName)
    {
        final DBI rootDbi = new DBI(rootConfig.getDBUrl(), rootConfig.getDBUser(), rootConfig.getDBPassword());
        final DBI userDbi = new DBI(userConfig.getDBUrl(), rootConfig.getDBUser(), rootConfig.getDBPassword());

        final boolean dbExists = rootDbi.withHandle(new HandleCallback<Boolean>() {
                @Override
        ...
        final boolean schemaExists = userDbi.withHandle(new HandleCallback<Boolean>() {
            @Override
            public Boolean withHandle(final Handle handle) {
                return handle.createQuery("SELECT count(*) FROM pg_namespace WHERE nspname=:schema")
    ...
    {
        final DatabaseController manager = DatabaseControllers.forPostgresSchema(rootConfig, userConfig);
